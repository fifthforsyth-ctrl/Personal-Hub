-- plan_summary bucketed logged time by UTC date, so an entry started at
-- 8pm Mountain (= 2am UTC the next day) counted toward the wrong day in
-- the month/year grids — the same local-vs-UTC trap the client-side date
-- helpers already avoid. The caller now passes its IANA zone.

drop function if exists public.plan_summary(date, date);

create function public.plan_summary(p_start date, p_end date, p_tz text default 'UTC')
returns table (
  day date,
  chunk_count integer,
  task_count integer,
